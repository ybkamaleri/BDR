...
preData_ut <- reactive({
  ...
  if (input$valg_data == 1) {
    if (input$valg_inspre == 1) {
      ...
      utDataKart <- preCount(preDiab, preNorge, byid = "rhf")
...
output[["kartDT"]] <- renderLeaflet({
  ...
  ## Sjekk hvis tallet til fordelinger dvs. pr.100 eller pr.10000 er ikke like
  ikkeUnik = sum(!duplicated(kartDT_ut()$pr.100))
  yesLeg = ifelse(ikkeUnik == 1, FALSE, TRUE)
  ...
  mapview(kartDT_ut(), zcol = "pr.100",
    popup = leafpop::popupTable(kartDT_ut(), zcol = c("region", "pr.100", "Case", "Populasjon"),
      feature.id = FALSE, row.numbers = FALSE), layer.name = txtLeg, legend = yesLeg)@map
  }

  if (input$valg_data == 1) {
  mapview(kartDT_ut(), zcol = "pr.100",
    popup = leafpop::popupTable(kartDT_ut(), zcol = c("helsereg", "pr.100", "Case", "Populasjon"),
      feature.id = FALSE, row.numbers = FALSE), layer.name = txtLeg, legend = yesLeg)@map
  }
