...
## -------------
## Antall i befolkning
NokBFrhf <- ssbKart[, .(N = sum(N, na.rm = TRUE)),
                 keyby = .(rhf, rhfNavn)][N != 0, ][]

## Type 1 for bare 0-14 år
dt14 <- subset(DTannon, age < 15)
dt14[, .N, keyby = rhf]
dt1FDrhf <- dt14[dbtype == 1, .N, keyby = rhf]


# prevalens HF
preCountHF <- function(x, y, per = 100, col = "prev", digit = 2){
  ...
  ## columns for fylkeid og mapID
  utCol <- grep("N$", names(preDT), value = T)

  ## tabCol <- names(preDT)[-utCol]
  tabName <- c("Case", "Populasjon", col)
  setnames(preDT, c(utCol, "ins"), tabName)
...
## Tabell
tabT1Drhf <- preCountHF(dt1FDrhf, NokBFrhf, per = 10000, col = "prev", digit = 1)
tabT1Drhf[]

## Landets prevalens
n = tabT1Drhf[, sum(Case)]
N = tabT1Drhf[, sum(Populasjon)]

n / N * 10000


## Gir text columns
tabT1Drhf[, txt := paste0(rhfNavn, " (", prev, ")")]

## convert to tbl_df
library(tibble)
tab1TBrhf <- as_tibble(tabT1Drhf)
tab1TBrhf
...
## basic map
library(ggplot2)
tk1 <- ggplot(kartT1D, aes(fill = prev)) +
   ## scale_color_brewer(palette = "Dark2") +
  scale_fill_gradient(low = "#99CCFF", high = "#000033") + 
  geom_sf()
...
tk2 <- tk1 +
  ...
  ## scale_fill_viridis_c() + #for viridis discrete data
...
## scale_fill_brewer - for discrete
## scale_color_brewer - for contineous
...
## legger point på kartet hvis ønskelig
tk2 + geom_sf(data = kart_centroid, aes(fill = prev), colour = "#CC0033")
...
(d1tKart <- ggplot(kartT1D, aes(fill = prev)) + geom_sf() + 
   ## scale_color_brewer(palette = "Dark2") +
   scale_fill_gradient(low = "#99CCFF", high = "#000033") + 
   geom_sf(data = kart_centroid, aes(fill = prev), colour = "#CC0033") + 
   geom_text_repel(data = subset(fylKord, mapID  %in% nordned), aes(X, Y, label = txt),
     fontface = "bold", size = 4, nudge_x = c(2, 3, 2, -3), nudge_y = c(-1.2, -0.5, -1.5, 1)) +
   geom_text_repel(data = subset(fylKord, mapID  %in% vest), aes(X, Y, label = txt),
     size = 4,  fontface = "bold", nudge_x = c(-3.5, -3, -3, -5.5), nudge_y = c(0.2, 0.5, 0.25, 0.8)) +
   ...
   theme_minimal() +
   guides(fill = guide_legend(title = "Prevalens T1D \npr.10,000 barn \nmellom 0-14 år \n(2018)")) +
   ## ggtitle("Prevalens T1D pr. 10,000 barn mellom 0-14 år") +
   theme(
...
ggsave("kartDiab1for0Til14.pdf", plot = d1tKart)
...
## --------
# merge kart og data
kartT1Drhf <- NorgeHFReg %>% left_join(tab1TBrhf, by = c("helsereg" = "rhf"))
kartT1Drhf

## Lage POINT istedenfor POLYGON
kart_centHF <- kartT1Drhf %>% sf::st_centroid() #create point
kart_centHF

# get coordinate X og Y for å lage text
fylKordRHF <- cbind(kart_centHF, st_coordinates(st_centroid(kartT1Drhf)))
fylKordRHF


## Lager kart
(d1tKartRHF <- ggplot(kartT1Drhf, aes(fill = rhfNavn)) + geom_sf() + 
   ## scale_color_brewer(palette = "Dark2") +
   ## scale_fill_gradient(low = "#99CCFF", high = "#CC0033") +
   scale_fill_viridis_d() + #for viridis discrete data
   ## geom_sf(data = kart_centHF, aes(fill = rhfNavn), colour = "#CC0033") + 
   geom_text_repel(data = fylKordRHF, aes(X, Y, label = txt),
     fontface = "bold", size = 4,
     nudge_x = c(7.5, 6, -5, 0),
     nudge_y = c(0.5, -0.5, -0.5, -1.5),
     segment.color = NA) +
   coord_sf(xlim = c(-5, 35), ylim = c(56, 73), expand = FALSE) +
   theme_minimal() +
   annotate("text", x = 5, y = 68, size = 4, 
     label = "Prevalens T1D pr.10,000 barn \nmellom 0-14 år i 2018 \n(For hele landet er 24 barn \npr. 10,000 barn)") + 
   ## guides(fill = guide_legend(title = "Prevalens T1D \npr.10,000 barn \nmellom 0-14 år \n(2018)")) +
   ## ggtitle("Prevalens T1D pr. 10,000 barn mellom 0-14 år") +
   theme(
     legend.position = "none",
...
d1tKartRHF
## ggsave("kartT1D0til14.png")
ggsave("kartDiabHFfor0Til14.png", plot = d1tKartRHF)
ggsave("kartDiabHFfor0Til14.pdf", plot = d1tKartRHF)
...
Her defineres det som nye oppdaget minus alle som har diabetes av alle typer.
...
insPas <- dt14[valg == 2, .(n = .N), keyby = .(fylkeid)]
sykPas <- dt14[valg == 1, .(ut = .N), keyby = .(fylkeid)]
sykPas
insPas
...
## for (j in seq_len(ncol(tabIns))){
##   set(tabIns, )
## }

tabName = names(tabIns)
tabIns[, (tabName) := lapply(.SD, function(x1) round(x1, digits = 1)),
  .SDcols = tabName]

## lager col for text
tabInsFinal <-  merge(fylID, tabIns, by = "mapID") #region
## Gir text columns
tabInsFinal[, txt := paste0(region, " (", ins10, ")")]

## tar bort col region siden det finnes på kartet allerede
tabInsFinal[, region := NULL]
tabInsFinal
...
## convert to tbl_df
library(tibble)
tabInsTB <- as_tibble(tabInsFinal)
tabInsTB
...
```{r}
# merge kart og data
kartT1DIns <- NorgeMapFylke %>% left_join(tabInsTB, by = "mapID")
kartT1DIns

## add points for kart
kart_centIns <- kartT1DIns %>% sf::st_centroid() #create point
kart_centIns

# get coordinate for fylke dvs. X og Y for å lage text
fylKordIns <- cbind(kart_centIns, st_coordinates(st_centroid(kartT1DIns)))
fylKordIns
...
## For justering og plasering av tekst
## de skal være i riktig rekkefølge!!
nordned = c(5, 10, 17, 50) # Finnmark, Nordland, Troms, Trøndelag
vest = c(7, 8, 13, 15) # Hordaland, Møre, Rogaland,Sogn
ssor = c(3, 16, 18, 19) # Aust-Agd, Telemark, Vest-Agder, Vestfold
...
## lager kart
(d1tKartIns <- ggplot(kartT1DIns, aes(fill = ins10)) + geom_sf() + 
   ## scale_color_brewer(palette = "Dark2") +
   scale_fill_gradient(low = "#99CCFF", high = "#000033") + 
   geom_sf(data = kart_centIns, aes(fill = ins10), colour = "#CC0033") + 
   geom_text_repel(data = subset(fylKordIns, mapID  %in% nordned), aes(X, Y, label = txt),
     fontface = "bold", size = 4, nudge_x = c(2, 3, 2, -3), nudge_y = c(-1.2, -0.5, -1.5, 1)) +
   geom_text_repel(data = subset(fylKordIns, mapID  %in% vest), aes(X, Y, label = txt),
     size = 4,  fontface = "bold", nudge_x = c(-3.5, -3, -3, -5.5), nudge_y = c(0.2, 0.5, 0.25, 0.8)) +
   geom_text_repel(data = subset(fylKordIns, mapID  %in% ssor), aes(X, Y, label = txt),
     size = 4,  fontface = "bold", nudge_x = c(2, 4, -3, 1), nudge_y = c(-1, -1, -0.15, -1)) +
   geom_text_repel(data = subset(fylKordIns, mapID  %in% sopp), aes(X, Y, label = txt),
     size = 4,  fontface = "bold", nudge_x = c(5.5, 3, 5), nudge_y = c(0.5, 0.25, 1.5)) +
   geom_text_repel(data = subset(fylKordIns, mapID  %in% oslo), aes(X, Y, label = txt),
     size = 4,  fontface = "bold", nudge_x = c(2.5, 4, 3), nudge_y = c(-0.1, 0.25, -0.2)) +
   coord_sf(xlim = c(0, 35), ylim = c(56, 73), expand = FALSE) +
   theme_minimal() +
   guides(fill = guide_legend(title = "Insidens T1D \npr.10,000 barn \nmellom 0-14 år \n(2018)")) +
   ## ggtitle("Prevalens T1D pr. 10,000 barn mellom 0-14 år") +
   theme(
     ## legend.position = "none",
     panel.grid = element_blank(),
...
d1tKartIns
## ggsave("kartT1D0til14.png")
ggsave("kartT1DInsfor0Til14.png", plot = d1tKartIns)
ggsave("kartT1DInsfor0Til14.pdf", plot = d1tKartIns)
