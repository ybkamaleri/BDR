...
DTAnoHF <- readRDS("DTannonHF.rds") #med HF
#Norges befolkning
ssbKart <- readRDS("SSBmedMapID.Rds")
#Kart med fylkeID som i SSB
NorgeMapFylke <- readRDS("NorgeMapFylkeID.Rds")
#Kart med fylke og helseregion
NorgeFylkReg <- readRDS("NorgeMapInt.Rds")
...
## saveRDS(NorgeHF, "NorgeMapInt.Rds")
...
# prevalens
preCount <- function(x, y, by = 100){

  ##by: per 100 person osv.
  ## x = DB data
  ## y = fylke data
  ...
  tabName <- c("Case", "Ins", "Populasjon")
  setnames(preDT, tabCol, tabName)

  preDT[, (tabName) := lapply(.SD, function(x1) round(x1, digits = 2)),
    .SDcols = tabName]
