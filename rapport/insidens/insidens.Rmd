...
list.of.packages <- c("data.table", "stringr", "lubridate", "shiny", "flexdashboard",
                      "leaflet","ggplot2","dplyr","sf","ggrepel", "tmap", "mapview", "ggrepel")
...
Disse datasetter er den endelige:

- **Data_withID_MapSSB.rds**: Befolkningstall med fylke og kart ID for kobling
- **dt2018.rds**: Datasett for diabetes med fylkeid
- **NorgeMapFylkeID**: Kart med fylkeid
...
norskBF18[, fylke := as.numeric(gsub("([0-9]+).*$", "\\1", region))]
...
ssbKart15 <- subset(ssbKart, age < 15)
## saveRDS(ssbKart15, "FylkeogKartID.Rds")
...
## Data path
dataSti <- "~/avid/bdr"
DT2018Alle <- readRDS(file.path(dataSti, "alleDiabetes2018.Rds"))
DT2018 <- subset(DT2018Alle, alder < 15) #bare de som er under 15 år

## Diabetes type, Fylkeid
pasientDT <- readRDS("./interaktiv/dt2018.rds") 
...
sykehusList <- DT[, .N, keyby = .(hosKort, hospID)][, N := NULL][]
sykehusList
...
DT2018Fyk <- sykID[DT2018, on = "hosKort"]
...
fylkSykKob <- DT2018Fyk[, .(hospID, fylkeid)]
## Join sykehus til fylke
fylkSykKob
## saveRDS(fylkSykKob, "sykehusFylker.Rds")
fylkSykKob[, .N, keyby = .(hospID, fylkeid)]
...
## saveRDS(fylkeList, "FylkeogKartID.Rds")
ssbKart <- readRDS("FylkeogKartID.Rds")
...
fylkeList[, .N, by = .(fylke, kartID, mixID, region)]
...
MapSsb <- ssbKart[, .(fylke, kartID, mixID, age, kjonn, N = get(nbf))]
...
```{r}
dt2018ANON <- DT2018[, .(alder, Kjonn, diabType, dbtype, fylkeid)]
## saveRDS(dt2018ANON, "dt2018.rds")
## saveRDS(dt2018ANON, file.path(dataSti, "AnnonymDBtype.rds"))


dd <- readRDS("./interaktiv/dt2018.rds")
dd[, age := floor(alder)] # ikke runder av f.eks 4.9 til 5 men til 4.
## setcolorder(dd, c("alder", "age", "Kjonn", "kjonn", "diabType", "dbtype", "fylkeid"))
## names(dd)
## saveRDS(dd, "./interaktiv/dt2018.rds")

```

# Statis kart

Lager vanlig kart som pdf eller png.
...
## add point
kart_centroid <- norgeMapFylke %>% sf::st_centroid() #create point
kart_centroid #her vises geometry som POINT

## g3 <- g2 + geom_sf(data = kart_centroid, aes(fill = region), colour = "red")
...
fylkeco <- cbind(kart_centroid, st_coordinates(st_centroid(norgeMapFylke)))
fylkeco #her ser man X og Y tilsvarer verdi i geometry

g2 <- ggplot(norgeMapFylke) +
  aes(fill = region) +
  scale_fill_viridis_d() + #viridis c for contineous
  geom_sf() + 
  geom_text(data = fylkeco, aes(X, Y, label = region), size = 8)  #legger text senere så det kan vises
