...
nbf <- names(norskBF)[4] #antall befolkningen
NokBF <- norskBF[, .(N = sum(get(nbf), na.rm = TRUE)),
                 keyby = fylke][N != 0, ][]

## Antall per kjønn
KjonnBF <- norskBF[, .(N = sum(get(nbf), na.rm = TRUE)),
                   keyby = .(kjonn, fylke)][N != 0, ][]

## Antall per alder
AgeBF <- norskBF[, .(N = sum(get(nbf), na.rm = TRUE)),
                 keyby = .(age, fylke)][N != 0, ][]

## Antall per alder og kjonn
AgeKjonnBF <- norskBF[, .(N = sum(get(nbf), na.rm = TRUE)),
                      keyby = .(kjonn, age, fylke)][N != 0, ][]
...
Lager function to rename columns og gir bare 2 digits.
...
renCol <- function(x){
  ...
  setnames(x, tabCol, tabName)

  x[, (tabCol) := lapply(.SD, function(x1) round(x1, digits = 1)),
    .SDcols = tabName]
...
preDT <- dtFD[NokBF, on = c(fylkeid = "fylke")]
preDT[, ins10 := round(N / i.N * 10000, digits = 1)] %>%
  .[, ins := round(N / i.N * 100, digits = 1)]
...
```{r}
preDT1 <- dt1FD[NokBF, on = c(fylkeid = "fylke")]
preDT1[, ins10 := round(N / i.N * 10000, digits = 1)] %>%
  .[, ins := round(N / i.N * 100, digits = 1)]
...
kartDT1 <- kart_sf %>% left_join(preDT1, by = c("fID" = "fylkeid"))
